    ...
    public class ContextComponentEditor : UnityEditor.Editor
    {
        ...
        private void DrawContextEdit()
        {
            if (GUILayout.Button("Edit"))
            {
                ...
                var scriptName = ContextComponent.ExpectedType.IsGenericType ?
                    ContextComponent.ExpectedType.Name.Split('`')[0] : 
                    ContextComponent.ExpectedType.Name;
                ...
                if (guids.Length > 0)
                {
                    foreach (var guid in guids)
                    {
                        ...
                        if (script != null &&  ContextComponent.ExpectedType.IsGenericType || script.GetClass() == ContextComponent.ExpectedType)
                        {
